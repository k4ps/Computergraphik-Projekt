function illuminate() {
    ...
    // var light2 = createLight(0x990000, 1);
    // light2.position.set (253.5, 116.9, -250);
    var light2 = createLighttube(0x6699ff, 0.5, 10, 90, 0, 90);
    light2.position.set (250, 80, -250);
    scene.add(light2);

    var light3 = createLight(0x990000, 0.1);
    light3.position.set(235, 83.5+10, -251.4);
    // scene.add(light3);
...
function createLighttube(color, radius, size, rot_x, rot_y, rot_z) {
    ...
    newObj.shadow.mapSize.width = 2048;
    newObj.shadow.mapSize.height = 2048;
    var geometry = new THREE.CylinderBufferGeometry(radius, radius, size, 32, 32);
    var material = new THREE.MeshBasicMaterial({ color: color });
    material.color.multiplyScalar(1.5);
    //material.transparent = true
    var sphere = new THREE.Mesh(geometry, material);
    sphere.rotation.set(THREE.Math.degToRad(rot_x), THREE.Math.degToRad(rot_y), THREE.Math.degToRad(rot_z));
    newObj.add(sphere);
    return newObj;
}
